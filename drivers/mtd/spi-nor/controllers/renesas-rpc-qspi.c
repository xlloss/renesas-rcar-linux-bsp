...
static int rpc_read_reg(struct spi_nor *nor, u8 opcode, u8 *buf, size_t len)
{
...
static int rpc_write_reg(struct spi_nor *nor, u8 opcode, const u8 *buf, size_t len)
{
	struct rpc_info *rpc = nor->priv;
	u32 val;
	int dsize;
...
static int rpc_erase_sector(struct spi_nor *nor, loff_t addr)
{
	...
	return nor->controller_ops->write_reg(nor, nor->erase_opcode, buf, nor->addr_width);
}

static const struct spi_nor_controller_ops rpc_qspi_ops = {
	.read = rpc_read_flash,
	.write = rpc_write_flash,
	.read_reg = rpc_read_reg,
	.write_reg = rpc_write_reg,
	.erase = rpc_erase_sector,
};
...
int rpc_qspi_probe(struct platform_device *pdev)
{
	...
	nor->controller_ops = &rpc_qspi_ops;
	nor->priv = rpc;
